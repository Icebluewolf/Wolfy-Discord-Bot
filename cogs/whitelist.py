...
class Whitelist(discord.Cog):
    ...
    async def whitelist(self, ctx, *, content):
        ...
        # Else Add Them To The Whitelist And Update Database
            ...
            # MCClient.client.respond_console_command(srv_id, "whitelist add " + content)
            await ctx.respond(embed=await global_functions.create_embed(title="Success",
                                                                     description=
    ...
    async def whitelist_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            await ctx.respond(embed=await global_functions.create_embed(title="error",
                                                                     description=
